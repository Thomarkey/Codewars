...
public class EqualSidesOfAnArray {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(findEvenIndex(new int[]{20, 10, 30, 10, 10, 15, 35})); // Output: 3
    }
    ...
    public static int findEvenIndex(int[] arr) {
        ...
        // Iterate through the array to find the index where left and right sums are equal
        for (int i = 0; i < arr.length; i++) {
            // Update the total sum by subtracting the current element
            totalSum -= arr[i];
            // Check if the left sum is equal to the updated total sum
            if (leftSum == totalSum) {
                return i;
            }
